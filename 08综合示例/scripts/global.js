...
function submitFormWithAjax(whichform, thetarget) {
  ...
  httpRequest.open("POST", whichform.getAttribute("action"), true);
  httpRequest.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
  httpRequest.onreadystatechange = function() {
    if(httpRequest.readyState == 4) {
      if(httpRequest.status == 200 || requestAnimationFrame.status ==0) {
        var matches = httpRequest.responseText.match(/<article>([\s\S]+)<\/article>/);
        if(matches.length > 0) {
        ...
        thetarget.innerHTML = "<p>" + httpRequest.statusText + "</p>";
      }
    }
  };
